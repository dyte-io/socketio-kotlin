...
class ConnectionTest : Connection("engine") {
    ...
    fun notSendPacketsIfSocketCloses() {
        ...
        socket.on(EngineSocket.EVENT_OPEN, handler = fun(_) {
            ...
            socket.on(EngineSocket.EVENT_PACKET_CREATE, handler = fun(_) {
                noPacket[0] = false
            })
            socket.close()
            socket.send("hi")
    ...
    fun deferCloseWhenUpgrading() {
        ...
        socket.on(EngineSocket.EVENT_OPEN, handler = fun(_) {
            ...
            socket.on(EngineSocket.EVENT_UPGRADE, handler = fun(_) {
                upgraded[0] = true
            })
            socket.on(EngineSocket.EVENT_UPGRADING, handler = fun(_) {
    ...
    fun closeOnUpgradeErrorIfClosingIsDeferred() {
        ...
        socket.on(EngineSocket.EVENT_OPEN, handler = fun(_) {
            ...
            socket.on(EngineSocket.EVENT_UPGRADE_ERROR, handler = fun(_) {
                upgradError[0] = true
            })
            socket.on(EngineSocket.EVENT_UPGRADING, handler = fun(_) {
                socket.on(EngineSocket.EVENT_CLOSE, handler = fun(_) {
    ...
    fun receivePing() {
        val values: BlockingQueue<Any> = LinkedBlockingQueue()
        var socket = EngineSocket(_opts = createOptions())
        socket.on(EngineSocket.EVENT_PING, handler = fun(_) {
            values.offer("end")
            socket.close()
        })
        socket.open()
