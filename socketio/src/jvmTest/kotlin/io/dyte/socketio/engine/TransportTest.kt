...
import io.dyte.socketio.engine.transport.TransportOptions
import io.dyte.socketio.engine.transport.WebSocketTransport
import io.dyte.socketio.engine.transport.XHRTransport
...
class TransportTest {
  ...
  fun uri() {
    ...
    opt.path = "/io.dyte.socketio.engine.io"
    opt.hostname = "localhost"
    opt.secure = false
    opt.query = Parameters.build { append("sid", "test") }
  ...
  fun wsUri() {
    ...
    opt.hostname = "test"
    opt.secure = false
    opt.query = Parameters.build { append("transport", "websocket") }
    opt.timestampRequests = false
    val ws = WebSocketTransport(opt)
    assertEquals("ws://test/io.dyte.socketio.engine.io?transport=websocket", ws.uri())
  }
  ...
  fun wssUri() {
    ...
    opt.timestampRequests = false
    val ws = WebSocketTransport(opt)
    assertEquals("wss://test/io.dyte.socketio.engine.io", ws.uri())
  }
  ...
  fun wsTimestampedUri() {
    ...
    val ws = WebSocketTransport(opt)
    assertTrue(
      ws.uri().matches(Regex("ws://localhost/io.dyte.socketio.engine.io\\?woot=[0-9A-Za-z-_.]+"))
    )
  }
  ...
  fun wsIPv6Uri() {
    ...
    opt.timestampRequests = false
    val ws = WebSocketTransport(opt)
    assert(ws.uri().contains("ws://[::1]/io.dyte.socketio.engine.io"))
  }
  ...
  fun wsIPv6UriWithPort() {
    ...
    opt.timestampRequests = false
    val ws = WebSocketTransport(opt)
    assert(ws.uri().contains("ws://[::1]:8080/io.dyte.socketio.engine.io"))
  }
}
